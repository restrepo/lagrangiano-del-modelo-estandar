...
Podemos ahora demostrar que la Acción obtenida del Lagrangiano en la ec.\eqref{eq:Lpr} es invariante bajo transformaciones de Lorentz haciendo uso de la ec.~(\ref{eq:Lambdacontra}). Para hacer la demostración más general,
\begin{frame}[fragile,allowframebreaks]
...
\begin{align}
  ...
  \partial_{0}\partial^{0}\phi + \sum_i\partial_{i}\partial^{i}\phi  =&0 \nonumber\\
        \partial_{0}\partial^{0}\phi - \sum_i\partial_{i}\partial_{i}\phi  =&0 \nonumber\\
...
\end{align}
y cómo en la ecuación~\eqref{eq:onda}
...
\begin{frame}[fragile,allowframebreaks]
Considere el campo espinorial, que se transforma como
\begin{align}
\label{eq:184qft}
...
\end{frame}
% En forma matricial, si $\xi(x)$ es un vector columna de dos componentes,
% \begin{align*}
%   \xi=
%   \begin{pmatrix}
%    \xi_1\\
...
% \begin{align}
%   \xi(x)\to \xi'(x)=S\xi \left(\Lambda^{-1}x  \right).
% \end{align}
% In such a case,
...
% % \begin{align}
% %     \xi^{\dagger}\to \xi'^{\dagger}=\xi^{\dagger}S^{\dagger}.
% % \end{align}
% En  componentes, y
% anticipando los índices con puntos
% para $S^{*}$, tenemos
% \begin{align}
%   \left( \xi'_\alpha \right)^{\dagger}=&\left( \xi_\beta \right)^{\dagger}{\left( S^\dagger\right)^{\dot{\beta}}}_{\dot{\alpha}}\nonumber\\
% =&\left( \xi_\beta \right)^{\dagger}{\left( S^*\right)_{\dot{\alpha}}}^{\dot{\beta}}\nonumber\\
% =&\left( \xi_\beta \right)^{\dagger}{\left( S^*\right)_{\dot{\alpha}}}^{\dot{\beta}}\,,
% \end{align}
\begin{frame}[fragile,allowframebreaks]
...
\end{align}
entonces tenemos que~\eqref{eq:antispinorright}
\begin{align}
...
\psi^{\dagger\dot{\beta}}{\left[ \left( S^{-1} \right)^{*}  \right]_{\dot{\beta}}}^{\dot{\alpha}}
  = &{\left[ \left( S^{\dagger} \right)^{-1} \right]^{\dot{\alpha}}}_{\dot{\beta}}\psi^{\dagger\dot{\beta}}\left( \Lambda^{-1}x \right),
\end{align}
...
\begin{align}
\psi\psi\equiv  \psi^{\alpha}\psi_{\alpha}\to {\psi'}^{\alpha}{\psi'}_{\alpha}=&\psi^{\beta} {\left[ \left( S^{-1} \right) \right]_{\beta}}^{\alpha} {S_\alpha}^\gamma\psi_\gamma \nonumber\\
...
\left( {\psi}^{\dagger}_{\dot{\beta}}  {S^{\dagger\dot{\beta}}}_{\dot{\alpha}} \right)\overline{\sigma}^{\mu\dot{\alpha\gamma}}{\left(\Lambda^{-1}\right)^\rho}_\mu\partial_\rho \left( {S_{\gamma}}^{\delta}\psi_\delta \right)\,.
 \end{align} 
 Como los parámetros de la transformación, $\xi_i$ y $\theta_i$, en la ec.~\eqref{eq:SLet} están en el espacio interno de Lorentz, 
 ni $\Lambda$, ni el cuadrivector de matrices constantes $\overline{\sigma}^{\mu}$ cambian bajo una transformación de Lorentz asociada a cambios de las coordenadas externas
\begin{align}
...
\end{align}
El término permanece invariante si la siguiente condición se satisface
\begin{align}
  \label{eq:preidentidadlag}
 {\left(\Lambda^{-1}\right)^\rho}_\mu  S^{\dagger}\overline{\sigma}^\mu S=\overline{\sigma}^\rho\,,
\end{align}
de modo que
\begin{align}
{\psi}^{\dagger}_{\dot{\alpha}}\overline{\sigma}^{\mu\dot{\alpha}\alpha}\partial_\mu\psi_{\alpha}\to  {\psi'}^{\dagger}_{\dot{\alpha}}\overline{\sigma}^{\mu\dot{\alpha}\alpha}\partial'_\mu\psi'_{\alpha}&=\psi^{\dagger}\overline{\sigma}^\rho\partial_\rho\psi\,.
\end{align}

Multiplicando matricialmente por $\Lambda$ a ambos lados de la ecuación~\eqref{eq:preidentidadlag}
\begin{align}
\label{eq:ltrincond}
...
Para garantizar que la Acción sea un número real, debemos imponer que  $\mathcal{L}^{\dagger}=\mathcal{L}$. La forma más simple de Lograrlo es simplemente adicionar el hermítico conjugado de cada uno de los términos (h.c de las siglas en inglés). De modo que el Lagrangiano más general posible para espinores de dos componentes es
\begin{align*}
...
\end{align*}
y usando la hermiticidad de las matrices de Pauli
\begin{align}
...
Ésta interpretación tiene consecuencias importantes porque implica que la teoría cuántica subyacente pues implica que la probabilidad se conserva de forma no local. 
\end{frame}
...
Una de las conexiones más hermosas y profundas entre la Teoría Clásica de Campos y la Mecánica Cuántica surge al estudiar las simetrías espaciotemporales de los espinores. 

En la sección anterior vimos que la simetría interna $U(1)$ (cambio de fase global) del campo espinorial da lugar, por el Primer Teorema de Noether, a una cuadricorriente conservada cuya componente temporal $j^0 = \psi^\dagger \psi$ interpretamos como una densidad de probabilidad. Si ahora aplicamos el \textbf{Primer Teorema de Noether para simetrías externas} (traslaciones en el espacio y en el tiempo), obtendremos el tensor de momento-energía $T^\mu_{\ \nu}$. Las cargas conservadas asociadas a este tensor son la energía total y el momentum total del campo.

Para el Lagrangiano libre de un espinor de Weyl izquierdo sin masa:
\begin{align}
  \label{eq:masslessweyl}
  \mathcal{L} =& i\psi^\dagger \bar{\sigma}^\mu \partial_\mu \psi \nonumber\\
  =& i\psi^\dagger \bar{\sigma}^0 \partial_0 \psi + i\psi^\dagger \bar{\sigma}^i \partial_i \psi \nonumber\\
=& i\psi^\dagger  \partial_0 \psi - i\psi^\dagger {\sigma}^i \partial_i \psi   \,,
\end{align}
podemos calcular la densidad Hamiltoniana (densidad de energía), que corresponde a la componente $T^0_{\ 0}$. Usando la definición del tensor de momento-energía y sabiendo que $\bar{\sigma}^0 = \mathbf{1}$, tenemos:
\begin{align}
    T^0_{\ 0} &= \frac{\partial \mathcal{L}}{\partial (\partial_0 \psi)} \partial_0 \psi + \partial_0 \psi^\dagger \frac{\partial \mathcal{L}}{\partial (\partial_0 \psi^\dagger)} - \mathcal{L} \nonumber \\
    &= i\psi^\dagger \partial_0 \psi - \mathcal{L} \, .
\end{align}
Usando~\eqref{eq:masslessweyl} 

\begin{align}
  T^0_{\ 0}   &= i\psi^\dagger \partial_0 \psi  - i\psi^\dagger  \partial_0 \psi + i\psi^\dagger {\sigma}^i \partial_i \psi \nonumber\\  \,,
  &= i\psi^\dagger (\sigma^i \partial_i) \psi \nonumber \\
    &= -\psi^\dagger \sigma^i (-i\partial_i) \psi \nonumber \\
    &= -\psi^\dagger (\boldsymbol{\sigma} \cdot \mathbf{\hat{p}})\psi \nonumber \\
    &= \psi^\dagger \hat{H}_W \psi \, ,
    \label{eq:T00_espinores}
\end{align}
donde hemos definido el \textbf{Hamiltoniano de Weyl} como:
\begin{equation}
    \hat{H}_W = -\boldsymbol{\sigma} \cdot \mathbf{\hat{p}} \, .
\end{equation}
(Este Hamiltoniano corresponde a la proyección del espín en la dirección opuesta al movimiento, justificando el signo menos para definir a $\psi$ como un espinor de helicidad \textit{izquierda}).

De la misma manera, la densidad de momentum físico del campo corresponde a las componentes espaciales $T^0_{\ i}$:
\begin{align}
    T^0_{\ i} &= \frac{\partial \mathcal{L}}{\partial (\partial_0 \psi)} \partial_i \psi \nonumber \\
    &= i\psi^\dagger \partial_i \psi \nonumber \\
    &= \psi^\dagger (-i\partial_i)\psi \nonumber \\
    &= \psi^\dagger \hat{p}_i \psi \, .
    \label{eq:T0i_espinores}
\end{align}

\subsubsection*{El surgimiento del postulado de la Mecánica Cuántica:}
A nivel clásico, la energía y el momentum total del sistema se obtienen integrando estas densidades de carga sobre todo el volumen del espacio. Si observamos con detenimiento el resultado de estas integrales utilizando las Ecs. \eqref{eq:T00_espinores} y \eqref{eq:T0i_espinores}:
\begin{align}
    E &= \int_V T^0_{\ 0} \, d^3x = \int_V \psi^\dagger \hat{H}_W \psi \, d^3x \, , \\
    \mathbf{P} &= \int_V T^0_{\ i} \, d^3x = \int_V \psi^\dagger \mathbf{\hat{p}} \psi \, d^3x \, ,
\end{align}
y recordamos que $\psi^\dagger \psi$ representa la distribución de probabilidad espacial del campo (normalizada a 1), nos damos cuenta de un hecho extraordinario: \textbf{la energía y el momentum clásico del campo se expresan matemáticamente como un promedio espacial ponderado por la densidad de probabilidad de la partícula}. 

En la Mecánica Cuántica tradicional, se introduce como un axioma o ``postulado'' impuesto ad-hoc que el valor esperado (o promedio estadístico) de un observable $\hat{O}$ se calcula como $\langle \hat{O} \rangle = \int \psi^\dagger \hat{O} \psi \, d^3x$. 

Sin embargo, como acabamos de demostrar, en el marco de la Teoría de Campos este no es un postulado independiente. Es una consecuencia directa e inevitable de exigir que la Acción del campo sea invariante ante traslaciones en el espacio y el tiempo (Primer Teorema de Noether). De modo que las cantidades conservadas clásicas son, por definición, los valores esperados cuánticos:
\begin{align}
    \langle \hat{H}_W \rangle &= \int_V \psi^\dagger \hat{H}_W \psi \, d^3x \, , \\
    \langle \mathbf{\hat{p}} \rangle &= \int_V \psi^\dagger \mathbf{\hat{p}} \psi \, d^3x \, .
\end{align}

\textit{Nota:} Como la ecuación de Schrödinger $i\partial_t \psi = \hat{H}_W \psi$ es de validez general en este contexto, la relación $\langle \hat{H} \rangle = \int \psi^\dagger \hat{H} \psi \, d^3x$ se mantiene. Sin embargo, cabe advertir que esta forma exacta construida solo con derivadas de primer orden no es posible para un Hamiltoniano de Weyl si se le añade un término de masa acoplado a un campo derecho, lo cual requerirá el formalismo de los espinores de Dirac.

Podemos definir el operador de cuadrimomento como
\begin{align}
  \label{eq:cuadrimomentum}
  \hat{p}^\mu = i\partial^\mu =& \left(i\partial^0,i\partial^i\right)\nonumber\\
  =& \left(i\partial_0,-i\partial_i\right)\nonumber\\
  =& \left(i\frac{\partial}{\partial t},-i \boldsymbol{\nabla}\right)\nonumber\\
   =& \left(\hat{H},\hat{\boldsymbol{p}}\right)\,.
\end{align}


Con estas definciones del operador $\hat{H}_W$ y $\hat{\boldsymbol{p}}$, podremos interpretar más facilmente la ecuaciones de Euler-Lagrange a continuación.
\end{frame}
...
\end{equation}
\end{frame}
con el Hamiltoniano de sólo derivadas de primer orden y covariante, en lugar del Hamiltoniano para el caso no relativista. 
...
\begin{equation}
  \label{eq:wspinorL}
    i\frac{\partial}{\partial t}\psi=-\boldsymbol\sigma\cdot\hat{\boldsymbol{p}}\psi.
\end{equation}
De la ec.~(\ref{eq:denshal})
\begin{equation}
  \label{eq:186qft}
  \hat{H}_{W}= -\boldsymbol\sigma\cdot\hat{\boldsymbol{p}}.
\end{equation}
\end{frame}
Si identificamos a $\boldsymbol{\sigma}/2$, como el operador de espín, el signo menos del Hamiltoniano quiere decir que el espín del campo $\psi$ se proyecta en la dirección opuesta a su momentum, lo que define una  \emph{helicidad}izquierda. 
De esta forma podemos interpretar el espinor de Weyl de dos components $\psi$ como un espinor de Weyl de \emph{quiralidad} izquierda, donde la quiralidad izquierda [derecha] se refiere a la propidedad del espinor de Weyl de pertencer a la representación $\left(\frac{1}{2},0\right)$ $\left[\left(\frac{1}{2},0\right)\right]$ del grupo de Lorentz. Note que la helicidad y la quiralidad coinciden sólo en el caso de masa $m$ (o en general para campos relativistas donde masa es despreciable en comparación con la energía del campo).   
...
\subsection{Invarianza de  Lorentz de la Acción de  Weyl}

\begin{frame}[fragile,allowframebreaks]
  Para mostrar que $S$ es de hecho una transformación de Lorentz, es conveniente escribir ésta de forma covariante. Si definimos
\begin{align}
  \sigma^{\mu\nu}=\frac{i}{4}\left[\sigma^\mu,\overline{\sigma}^\nu\right]\,.
\end{align}
Podemos obtener los generadores de boost y rotaciones apropiados:
\begin{align*}
...
% \end{align}
Vale la pena notar el hecho que $\sigma^{\mu\nu}$ satisface el álgebra de Lorentz y por consiguiente son los generadores del grupo de Lorentz:
\begin{align}
...
\begin{table}
  ...
  \begin{tabular}{llll}
   ...
   $e_R$: electrón {\color{blue}derecho}   & $\left( \eta^{\color{red}\alpha} \right)^{\dagger}=\eta^{\dagger\;\color{blue}\dot{\alpha}}$ & ${\left[ \left( S^{\dagger} \right)^{-1} \right]^{\color{blue}\dot{\alpha}}}_{\color{blue}\dot{\beta}}\eta^{\dagger\;\color{blue}\dot{\beta}}$& $e^{i\theta}\eta^{\dagger\;\color{blue}\dot{\alpha}}$\\
...
\end{pmatrix}
= \begin{pmatrix}                                
\left(e_L\right)^\dagger   & \left(e_R\right)^\dagger \\
  \end{pmatrix}                                
\end{align}
...
\begin{pmatrix}
  ...
  \end{pmatrix}\\
=& \begin{pmatrix}
  \eta^{\alpha}\sigma^{\mu}_{\alpha\dot{\alpha}}&
  \xi^{\dagger}_{\dot{\alpha}}\overline{\sigma}^{\mu\;\dot{\alpha}\alpha}
\end{pmatrix}\\
=& \begin{pmatrix}
  \left(e_R\right)^\dagger \sigma^{\mu}&
\left(e_R\right)^\dagger  \overline{\sigma}^{\mu}
\end{pmatrix}
\end{align}
...
\begin{frame}[fragile,allowframebreaks]
\section{El Campo de Dirac y el Operador de Carga Eléctrica}

En contraste con el campo escalar, podemos realizar una interpretación formal directa de la corriente en el Lagrangiano de Dirac utilizando generadores de grupo. Considere el Lagrangiano libre:
\begin{equation}
    \mathcal{L} = \bar{\psi}(i\gamma^\mu \partial_\mu - m)\psi \, .
\end{equation}

Definamos la transformación de fase global $U(1)$ en términos de un \textbf{operador de carga eléctrica} $\hat{Q}$. La transformación sobre el espinor se escribe como:
\begin{equation}
    \psi \to \psi' = e^{i\theta \hat{Q}} \psi \quad \implies \quad \delta\psi = i\theta \hat{Q}\psi \, ,
\end{equation}
donde $\theta$ es un parámetro infinitesimal. En el formalismo de campos, exigimos que la aplicación del operador $\hat{Q}$ sobre el campo entregue su respectivo autovalor de carga. Si el campo $\psi$ describe un fermión con carga $q$, entonces:
\begin{equation}
    \hat{Q}\psi = q\psi \, .
\end{equation}

Para que el Lagrangiano sea invariante ($\bar{\psi}'\psi' = \bar{\psi}\psi$), el espinor adjunto debe transformarse con la fase opuesta:
\begin{equation}
    \bar{\psi} \to \bar{\psi}' = \bar{\psi} e^{-i\theta \hat{Q}} \quad \implies \quad \delta\bar{\psi} = -i\theta \bar{\psi} \hat{Q} \, .
\end{equation}
Dado que el campo adjunto $\bar{\psi}$ opera creando partículas (o aniquilando antipartículas), su comportamiento bajo el operador de carga resulta en el autovalor opuesto, garantizando el balance algebraico de la simetría.

Aplicando el Teorema de Noether, la corriente conservada asociada a este generador es:
\begin{align}
    j^\mu &= \frac{\partial \mathcal{L}}{\partial(\partial_\mu \psi)} \frac{\delta\psi}{\theta} \nonumber \\
    &= (i\bar{\psi}\gamma^\mu) (i\hat{Q}\psi) \nonumber \\
    &= -\bar{\psi}\gamma^\mu \hat{Q}\psi \, .
\end{align}

Sustituyendo el autovalor del campo ($\hat{Q}\psi = q\psi$), obtenemos:
\begin{equation}
    j^\mu = -q \bar{\psi}\gamma^\mu \psi \, .
\end{equation}
Si el campo describe al electrón, su carga es $q = -e$. La cuadricorriente toma entonces su forma definitiva y célebre en la Electrodinámica Cuántica (QED):
\begin{equation}
    j^\mu_{EM} = e \bar{\psi}\gamma^\mu \psi \, .
\end{equation}

\subsubsection*{Interpretación Ontológica:} 
En la mecánica cuántica no relativista, se acostumbraba a interpretar la componente $j^0 = \psi^\dagger \psi$ puramente como la probabilidad de hallar a la partícula. Sin embargo, al incorporar el operador de carga $\hat{Q}$ de forma rigurosa en el Teorema de Noether, la densidad temporal toma la forma $j^0 = \psi^\dagger \hat{Q} \psi$. 

Esta formulación obliga a reinterpretar la magnitud conservada. Ya no estamos ante una corriente de probabilidad, sino ante una genuina \textbf{cuadricorriente de carga eléctrica}. Esta transición es conceptualmente obligatoria en la Teoría Cuántica de Campos, ya que en un régimen donde partículas y antipartículas pueden ser creadas y aniquiladas, el número de partículas (probabilidad) no se conserva; lo que el Universo preserva de forma estricta y fundamental a nivel local es el balance de sus cargas internas.
...
\begin{enumerate}
 ...
 \item Demostrar que el Lagrangiano
   \begin{align*}
      \mathcal{L}=& i\psi^\dagger\overline{\sigma}^\mu\partial_\mu\psi -m \left( \psi\psi+\psi^{\dagger}\psi^{\dagger} \right),
   \end{align*}
 es real hasta una derivada total, es decir que
 \begin{align*}
   \mathcal{L}^\dagger = \mathcal{L}+\partial_\mu \left(X^\mu\right),
 \end{align*}
 para un cierto $X^\mu$ a calcular.
...
\vspace{0.5cm}
\noindent \textbf{Ejercicio: El Teorema de Noether y los postulados de la Mecánica Cuántica}

\noindent Considere la densidad Lagrangiana de Dirac para un campo fermiónico libre de masa $m$:
\begin{equation*}
    \mathcal{L} = \bar{\psi}(i\gamma^\mu \partial_\mu - m)\psi
\end{equation*}
donde $\psi$ es un espinor de Dirac de cuatro componentes y $\bar{\psi} = \psi^\dagger \gamma^0$ es su espinor adjunto. Este ejercicio demuestra cómo los postulados de la mecánica cuántica emergen de manera natural a partir de las simetrías clásicas de este campo.

\begin{enumerate}
    \item \textbf{Ecuaciones de movimiento y el Lagrangiano \textit{on-shell}:} 
    Utilice las ecuaciones de Euler-Lagrange para el campo $\bar{\psi}$ para derivar la ecuación de movimiento (la ecuación de Dirac). Utilizando este resultado, demuestre que cuando el campo satisface las leyes de la física (es decir, \textit{on-shell}), la densidad Lagrangiana se anula localmente ($\mathcal{L} = 0$). \textit{Nota: Verá que esta es una propiedad excepcional de los campos fermiónicos que son lineales en las derivadas.}

    \item \textbf{Simetría Interna (Conservación de la Probabilidad):} 
    Suponga una transformación de fase global $U(1)$ dada por $\psi \to e^{-i\alpha}\psi$ y $\bar{\psi} \to \bar{\psi}e^{i\alpha}$, donde $\alpha$ es un parámetro infinitesimal constante.
    \begin{enumerate}
        \item Utilice el Primer Teorema de Noether para simetrías internas ($j^\mu = \sum \frac{\partial \mathcal{L}}{\partial(\partial_\mu\phi_i)} a_i$) para encontrar la cuadricorriente conservada $j^\mu$.
        \item Demuestre que la densidad de carga $j^0$ se reduce a $\psi^\dagger \psi$. Concluya por qué la conservación de la carga total $Q = \int j^0 d^3x$ justifica el postulado mecánico-cuántico que interpreta a $\psi^\dagger \psi$ como una densidad de probabilidad.
    \end{enumerate}

    \item \textbf{Simetría Externa (Tensor Momento-Energía):} 
    La invariancia del Lagrangiano bajo traslaciones espaciotemporales ($x^\mu \to x^\mu + \delta x^\mu$) define el tensor de momento-energía clásico según el Teorema de Noether:
    \begin{equation*}
        T^\mu_{\ \nu} = \frac{\partial \mathcal{L}}{\partial(\partial_\mu \psi)}\partial_\nu \psi - \delta^\mu_\nu \mathcal{L}
    \end{equation*}
    \begin{enumerate}
        \item Utilice el resultado del inciso (a) para simplificar este tensor a su forma evaluada en las ecuaciones de movimiento (\textit{on-shell}).
        \item Calcule la componente $T^0_{\ 0}$, que corresponde a la \textbf{densidad de energía} clásica del campo. Utilizando la ecuación de Dirac $i\partial_0 \psi = \hat{H}_D \psi$ (donde $\hat{H}_D = -i\boldsymbol{\alpha}\cdot\nabla + \beta m$ es el Hamiltoniano), demuestre que $T^0_{\ 0} = \psi^\dagger \hat{H}_D \psi$.
        \item Calcule las componentes espaciales $T^0_{\ i}$, que corresponden a la \textbf{densidad de momentum} físico del campo. Usando el operador cuántico de momento $\mathbf{\hat{p}} = -i\nabla$, demuestre que el vector de densidad de momentum es $\mathbf{T}^0 = \psi^\dagger \mathbf{\hat{p}} \psi$.
    \end{enumerate}

    \item \textbf{El postulado de los Valores Esperados:} 
    En los cursos introductorios de Mecánica Cuántica, se postula \textit{ad hoc} que el valor esperado (medible) de un observable $\hat{O}$ se calcula como $\langle \hat{O} \rangle = \int \psi^\dagger \hat{O} \psi \, d^3x$. 
    
    A partir de sus resultados en el inciso (c) para la energía total clásica $E = \int T^0_{\ 0} d^3x$ y el momentum total clásico $\mathbf{P} = \int \mathbf{T}^0 d^3x$, argumente física y ontológicamente por qué este postulado de la mecánica cuántica no es arbitrario, sino una consecuencia ineludible de la Teoría Clásica de Campos y del Teorema de Noether.
