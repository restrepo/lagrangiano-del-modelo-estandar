...
Como $a^{0}=\mathbf{1}$,
\begin{equation}
    i\frac{\partial}{\partial t}\psi=(\boldsymbol{a}\cdot\mathbf{p}+m)\psi.
\end{equation}
...
En términos de la matrices $\gamma^\mu$, el Lagrangiano de Weyl y la ecuación de Weyl, son respectivamente de las ecs.~(\ref{eq:100qft}) y (\ref{eq:114qft})

%Aclar que es sigma mu
\begin{equation}
  \label{eq:115qft}
  \mathcal{L}=\psi^\dagger\left(i\gamma^\mu\partial_\mu\right)\psi,
\end{equation}
\begin{equation}
  \label{eq:116qft}
