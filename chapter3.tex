...
Vamos a comenzar con la construcción de Lagrangiano para el espinor de Weyl de índice abajo $\psi_{\alpha}=\xi_{\alpha}$, usando el cudrivecto de Lorentz con estructura tensorial en $\operatorname{SL}(2,C)$ $\overline{\sigma}^{\mu\dot{\alpha}\beta}$
...
Para el campo de dos componentes derecho  ${\eta^{\dagger}}^{\dot{\alpha}}$ ($\dot{\alpha}=\dot{1},\dot{2}$), el término invariante de Lorentz con derivada de primer orden debería ser
 \begin{align}
   {\eta}\sigma^\mu\partial_\mu\eta^{\dagger}\to  {\eta'}\left(\sigma^\mu\right)\partial'_\mu{\eta^{\dagger}}'
...
\begin{table}
  ...
  \begin{tabular}{llll}
   ...
   $\left( e_R \right)^{\dagger}$: positrón {\color{red}izquierdo}&$\eta^{\color{red}\alpha}$& $\eta^{\color{red}\alpha}{\left[ S^{-1}  \right]_{\color{red}\beta}}^{\color{red}\alpha}$ & $e^{-i\theta}\eta^{\color{red}\alpha}$\\\hline
  \end{tabular}
