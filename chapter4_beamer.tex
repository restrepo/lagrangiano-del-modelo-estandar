...
\begin{figure}
  \centering
  \includegraphics[scale=1]{metalcristal1}
...
\begin{figure}
  \centering
  \includegraphics[scale=1]{metalcristal2}
  \caption{En la figura (a), los círculos con líneas a trazos representan las posiciones originales de los iones positivos en el cristal. Debido a la presencia del electrón de valencia, el cristal se polariza y se genera un exceso de carga positiva alrededor del electrón que resulta entonces apantallado. En la figura (b) se muestra como el exceso de carga positiva puede causar un efecto de atracción sobre otro electrón de valencia lo que permite la formación de un par de Cooper.}
  \label{fig:1}
\end{figure}
...
\begin{align*}
  \phi(x)=\frac{\varphi(x)}{\sqrt{2}}e^{i\eta(x)}
\end{align*}
...
\begin{align*}
     \phi(x)\to\phi'(x)=e^{i\theta(x)}e^{i\eta(x)}\frac{\varphi(x)}{\sqrt{2}}\,.
\end{align*}
Haciendo $\theta(x)=-\eta(x)$,
\begin{align}
\label{eq:87qft}
   \phi(x)\to\phi'(x)=&e^{-i\eta(x)+i\eta(x)}\frac{\varphi(x)}{\sqrt{2}}=\frac{\varphi(x)}{\sqrt{2}} \nonumber\\
...
\begin{align}
 ...
 &=\tfrac{1}{2}\left[\partial^\mu \varphi+ig{A'}^\mu \varphi\right]\left[\partial_\mu \varphi-ig{A'}_\mu \varphi\right]-\tfrac{1}{2}\mu^2 \varphi^2-\tfrac{1}{4}\lambda \varphi^4-\tfrac{1}{4}\left(F^{\mu\nu}F_{\mu\nu}\right)'\,.
\end{align}
Concentremenos en la parte que llamaremos potencial escalar
\begin{align}
...
\begin{figure} %noinstiki
  ...
  \caption{$V(\varphi)=\frac{1}{2}\mu^2 \varphi^2$ con $\mu^2\gt 0$} %noinstiki
...
\begin{equation}
  V(\varphi)=\tfrac{1}{2}\mu^2\varphi^2+\tfrac{1}{4}\lambda\varphi^4
  \qquad   \mu^2\lt 0,\ \lambda\gt 0
\end{equation}
que mantiene la simetría bajo la transformación discreta $\varphi\to-\varphi$. $\lambda\gt 0$ garantiza la aparición de los dos mínimos que se muestran el la figura \ref{fig:x2l}. Si la energía es suficientemente alta como se muestra en la figura~\ref{fig:x2l}, las excitaciones son simétricas con respecto al máximo del potencial y el término en $\mu^2$ no puede interpretarse como masa para la partícula escalar. 

%noinstiki
\begin{figure} %noinstiki
  \centering %noinstiki
  \includegraphics[scale=0.8]{vphi4}
  \caption{$V(\varphi)=\frac{1}{2}\mu^2 \varphi^2+\frac{1}{4}\lambda\varphi^4$ con $\mu^2\lt 0$, y $\lambda\gt 0$. Simetría exácta} %noinstiki
...
\begin{figure} %noinstiki
  \centering %noinstiki
  %noinstiki
  \includegraphics[scale=0.8]{vphi4m}
  \caption{$V(\varphi)=\frac{1}{2}\mu^2 \varphi^2+\frac{1}{4}\lambda\varphi^4$ con $\mu^2\lt 0$, y $\lambda\gt 0$. Simetría espontáneamente rota.} %noinstiki
...
Sin embargo, si la energía es suficientemente baja como se muestra en la figura~\ref{fig:x2lm}, las excitaciones alrededor del mínimo dan lugar a la aparición de un término de masa para el campo escalar. Además, dichas excitaciones no respetan la simetrías $\varphi\to-\varphi$. En tal caso decimos que la simetría ha sido espontáneamente rota: aunque el Lagrangiano mantiene la simetría original, el vacío la rompe. 


Para analizar cuantitativamente el espectro de partículas es necesario expandir el campo alrededor del mínimo y determinar las excitaciones. Establezcamos en primer lugar los mínimos del potencial. La $\partial V/\partial\varphi=0$ da lugar a
\begin{align}
  \mu^2\varphi+\lambda\varphi^3&=0\\
  \varphi(\mu^2+\lambda\varphi^2)&=0,
\end{align}
con extremos $\varphi_{\text{max}}=0$, y 
\begin{equation}
  \label{eq:90qft}
  \varphi_{\text{min}}\equiv\langle\varphi\rangle\equiv v=\pm\sqrt{\frac{-\mu^2}{\lambda}}.
\end{equation}
De hecho 
\begin{equation}
  \frac{\partial^2V}{\partial\varphi^2}=\mu^2+3\lambda\varphi^2.
\end{equation}
$\varphi=0$ corresponde a un máximo, mientras que la segunda derivada para $\varphi=\pm\sqrt{-\mu^2/\lambda}$ es $-2\mu^2\gt 0$ y corresponden a los mínimos. Expandiendo el campo alrededor del mínimo
\begin{equation}
  \varphi(x)=H(x)+v
\end{equation}
\begin{align}
...
\begin{align}
  V(\varphi)&=-\tfrac{1}{2}\lambda v^2\varphi^2+\tfrac{1}{4}\lambda\varphi^4\nonumber\\
...
Expandiendo el campo $\varphi(x)$ alrededor del mínimo
\begin{align}
  \varphi(x)\to H(x)+v\,.
\end{align}
\begin{align}
  \mathcal{L}\to\mathcal{L}' =&\left[\left(\mathcal{D}^\mu\right)'\varphi'\right]^*\left(\mathcal{D}_\mu\right)'\varphi'-\mu^2\left(\varphi^*\right)'\varphi'-\lambda\left[\left(\varphi^*\right)'\varphi'\right]^2-\tfrac{1}{4}\left(F^{\mu\nu}F_{\mu\nu}\right)'\nonumber\\
 =&\tfrac{1}{2}\left[\partial^\mu H+ig{A'}^\mu(H+v)\right]\left[\partial_\mu H-ig{A'}_\mu(H+v)\right] \nonumber\\
&-\tfrac{1}{2}\mu^2(H+v)^2-\tfrac{1}{4}\lambda(H+v)^4-\tfrac{1}{4}\left(F^{\mu\nu}F_{\mu\nu}\right)'\,.
\end{align}
