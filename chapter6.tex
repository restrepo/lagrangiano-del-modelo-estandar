...
En el caso de un doblete fermiónes $\Xi$, el símbolo  $\dagger$ se usa para denotar el conjugado de fermiones de Weyl. El doblete adjunto en ese caso se puede definir como\begin{align}
\label{eq:Xiadj}
...
Para evitar confusón con el conjugado de spinores de Weyl, usaremas el producto escalar con la métrica $SU(2)$ para escribir los correspondientes invariantes.
...
All the particles in this Lagrangian are massless. It is only good for
the gluons and the Abelian gauge boson, but is no realist for the
fermions of the weak gauge bosons $W_\mu^i$. To solve this problem, we
postulate a new complex scalar doublet, con su correspondiente adjunto de $SU(2)$ with four degree of freedom:
\begin{align}
  \Phi=&
  \begin{pmatrix}
    \phi^0\\
...
\widetilde{\Phi}=&
  ...
  \end{pmatrix}\,.
\end{align}
The ``$\mp$'' and 0 superindexes are just for later convenience, with $\left( \phi^- \right)^{*}=\phi^+$
...
\begin{align}
 \eta_1\,\Xi_1^{a}\cdot\Phi+\left(\eta_1\,\Xi_1^{a}\cdot\Phi  \right)^{\dagger}
 =& \eta_1\,\Xi_1^{a}\cdot\Phi+\text{h.c}\nonumber\\
 =& \eta^{b}_1\,\epsilon_{ab}\,\Xi_1^{a}\cdot\Phi^{b}+\text{h.c} \nonumber\\
   =&\left( e_R \right)^{\dagger}\epsilon_{ab}L^a\Phi^b +\text{h.c} \nonumber\\
   =&\left( e_R \right)^{\dagger}L^1\Phi^2-\left( e_R \right)^{\dagger}L^1\Phi^2 +\text{h.c} \nonumber\\
...
\nonumber\\
     ...
     &- \left[  h_e \left( e_R \right)^{\dagger}\,\epsilon^{ab}L_a\Phi_b +
      h_d \left( d_R \right)^{\dagger}\,\epsilon^{ab}Q_a\Phi_b +
      h_u \left( u_R \right)^{\dagger}\,\epsilon_{ab}Q_a\widetilde{\Phi}_{b}+\text{h.c}\right]\nonumber\\
...
\mathcal{L}_{\text{gauge}}=& -\tfrac{1}{4}G^{\mu\nu}_a G_{\mu\nu}^a-\tfrac{1}{4}W^{\mu\nu}_i W_{\mu\nu}^i-\tfrac{1}{4}B^{\mu\nu} B_{\mu\nu}\nonumber\\
  \mathcal{L}_{WBH}=&\left( \mathcal{D}_\mu\widetilde{\Phi} \right)\cdot\mathcal{D}^\mu\Phi-\mu^2\widetilde{\Phi}\cdot\Phi-\lambda \left( \widetilde{\Phi}\cdot\Phi \right)^2 \nonumber\\
...
\mathcal{L}_{\text{Yukawa}}=&- \left[  h_e \left( e_R \right)^{\dagger}\,\epsilon^{ab}L_a\Phi_b +
      h_d \left( d_R \right)^{\dagger}\,\epsilon^{ab}Q_a\Phi_b +
      h_u \left( u_R \right)^{\dagger}\,\epsilon_{ab}Q_a\widetilde{\Phi}_{b}+\text{h.c}\right]
\end{align}
...
\begin{align}
  V(\Phi)=-\mu^2\widetilde{\Phi}^\dagger\widetilde{\Phi}-\lambda(\widetilde{\Phi}^\dagger\widetilde{\Phi})^2
\end{align}
se reduce a
\begin{align}
...
\begin{equation}
  \label{eq:123qft}
    \widetilde{\Phi}\to\widetilde{\Phi}'=
  \begin{pmatrix}
    0\\
    \frac{1}{\sqrt{2}}(H(x)+v)
  \end{pmatrix},
\end{equation}
que define el \emph{gauge unitario}. En adelante sin embargo omitiremos las primas sobre los campos transformados $\widetilde{\Phi}'$ y $W'_{\mu\nu}$.
...
Entonces
\begin{align}
\mathcal{D}_\mu\widetilde{\Phi}=\begin{pmatrix}
...
\begin{align}
  \label{eq:lwbhfin}
  \mathcal{L}_{W B H}=&\left(\mathcal{D}^\mu\widetilde{\Phi}\right)^\dagger\mathcal{D}_\mu\widetilde{\Phi}-\mu^2\widetilde{\Phi}^\dagger \widetilde{\Phi}-\lambda\left(\widetilde{\Phi}^\dagger\widetilde{\Phi}\right)^2\nonumber\\
...
\begin{align}
  \mathcal{L}_{\text{Yukawa}}=&h_e \widetilde{L}\cdot\Phi e_R+h_d \widetilde{Q}\cdot \Phi d_R+h_u Q^{\dagger}\cdot\widetilde{\Phi}u_R+\text{h.c}\nonumber\\
...
\begin{align}
  \label{eq:azl}
      \mathcal{L}_{A Z L}\to\sum_{F=Q,L,e_R^{\dagger},d_R^{\dagger},u_R^{\dagger}}\frac{e}{2c_W s_W}\widetilde{F}\cdot\overline{\sigma}^\mu\left[ \tau_3-2s_W^2\widehat{Q}_L\right]F Z_\mu
       +e\widetilde{F}\cdot\overline{\sigma}^\mu \widehat{Q}_L F A_\mu\,.
\end{align}
con
\begin{align}
  \widetilde{F}\cdot \to F^{\dagger} \qquad \text{for}\quad F=(e_R)^{\dagger},(d_R)^{\dagger},(u_R)^{\dagger}\,.
\end{align}
...
\begin{align}
\label{eq:sm1g2}
 \mathcal{L}_{\text{fermion}}=&i Q^{\dagger}\overline{\sigma}^\mu\mathcal{D}_\mu Q+i \widetilde{L}\cdot\overline{\sigma}^\mu\mathcal{D}_\mu L+
...
&+\frac{g}{\sqrt{2}}\left[{\nu_L}^{\dagger}\overline{\sigma}^\mu e_LW_\mu^++
{u_L}^{\dagger}\overline{\sigma}^\mu d_LW_\mu^++\text{h.c}\right]\nonumber\\
&+\sum_{F=Q,L,e_R,d_R,u_R}\frac{e}{2c_W s_W}F^{\dagger}\overline{\sigma}^\mu\left[ \tau_3-2s_W^2\widehat{Q}_L\right]F Z_\mu\nonumber\\
...
%%arrglar
\begin{align}
  \mathcal{L}_{\text{1 gen}}=&\sum_f i{f}^{\dagger}\left(\overline{\sigma}^\mu\partial_\mu-m_f\right)f\nonumber\\
&-\tfrac{1}{4}F^{\mu\nu} F_{\mu\nu}-\tfrac{1}{4}Z^{\mu\nu} Z_{\mu\nu}-\tfrac{1}{2}(F_W^\dagger)^{\mu\nu} (F_W)_{\mu\nu}
- \tfrac{1}{4}\widetilde{G}^{\mu\nu}_a \widetilde{G}_{\mu\nu}^a\nonumber\\
