...
\section{interacciones Fermión-gauge }
...
\section{Interacciónes débiles fermiónicas para leptones}
...
\end{align}
De esto modo, la simetría gauge genera la interacción deseada con los bosones gauge cargados $W_{\mu}^{\pm}$. Sin embargo, se generan muchas otras interacciones las cuales deben contener apropiadamente la interacción electromagnética en algún cambio de base.
...
\end{align}
donde $e$ es la carga eléctrica del electrón, y $\widehat{Q}$ es el generador de carga elétrica conservada asociada a la simetría gaugel local $\operatorname{U}(1)_Q$. 
...
\begin{align}
  \label{eq:azl}
      \mathcal{L}_{A Z L}\to\sum_{F=Q,L,e_R,d_R,u_R}\frac{e}{2c_W s_W}\widetilde{F}\cdot\overline{\sigma}^{\mu}\left[ \tau_3-2s_W^2\widehat{Q}\right]F Z_\mu
       +e\widetilde{F}\cdot\overline{\sigma}^\mu \widehat{Q} F A_\mu\,.
\end{align}
con
\begin{align}
...
\begin{equation}
  ...
  \end{pmatrix},
\end{equation}
con $\tan\theta_W={g_1}/g$, tal que $g\sin\theta_W={g_1}\cos\theta_W$, como en la ec.~\eqref{eq:82qft}. Note que $V$ es una matrix ortogonal que satisface $VV^T=V^TV=\mathbf{1}$. Si (ver ec.~\eqref{eq:81qft}),
\begin{align}
  \begin{pmatrix}
    W^3_\mu\\
    B_\mu
...
&+\frac{g_2}{2\sqrt{2}}\left[\sum_{f}\bar{f}\gamma^\mu(1-\gamma_5)f' W_\mu^++\text{h.c}\right]
