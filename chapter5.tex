...
La derivada covariante se define de manera que transforme de la misma forma que el campo,
% introduciendo el campo gauge $A^\mu$
% \begin{equation}
%   \label{eq:202qft}
...
\end{align}
como corresponde a una derivada covariante. Tal que la derivada normal sufra una sustitución mínima, es decir, debemos introducir el campo gauge $A^\mu(x)$
\begin{equation}
  \label{eq:202qft}
  \partial_\mu\to\mathcal{D}_\mu=\partial_\mu-ieQA_\mu\,,
\end{equation}
...
\begin{align}
    \left( \partial_{\mu}-ieQ A'_{\mu} \right)\left(U\psi\right) =&U\partial_{\mu}\psi-ie  U A_{\mu}Q\psi \nonumber\\
...
\begin{align}
      QA'_{\mu}U =&\frac{i}{e}\left( \partial_{\mu}U \right) + U QA_{\mu} \nonumber\\
...
\end{align}
Está expresión será de validez general incluso para Grupos no Abelianos con la combinación de generadores y campos $QA_{\mu}$. Esta expresión es completamente general y será usada posteriormente en el contexto de grupos más complicados. En el caso particular de un Grupo Abeliano  $U(1)$, tenemos simplemente que
\begin{align}
  UQA_{\mu}U^{-1}=QA_{\mu}\,,
\end{align}
de modo que, cancelando el generado $Q$ a ambos lados de la igualdad
\begin{align}
...
\end{align}
donde habíamos establecido, ver ec.~\eqref{eq:cuadriconj}, que $j^{\mu}$ debe ser una cuadricorriente conservada
\begin{align}
  \partial_{\mu}j^{\mu}=0\,.
\end{align}
