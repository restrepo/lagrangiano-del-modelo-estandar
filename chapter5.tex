...
\subsection{Construcción del tensor cromodinámico a partir de la derivada covariante}
\label{sec:construccion_tensor_color}
...
\subsubsection*{1. El fracaso del tensor electromagnético ingenuo}
...
Bajo una transformación gauge local $SU(3)_c$, parametrizada por $\theta_a(x)$, 
...
\end{align}
El término entre paréntesis es exactamente la definición de la derivada covariante en la representación adjunta $8\times 8$ de componentes (ver ec.~\eqref{eq:adjunta})
1\begin{align}
...
y
\begin{align}
   \mathcal{D}_\mu = \delta^{ac}\partial_\mu\mathcal{E}^\mu_c \ -i g_s (\widetilde{\Lambda}_b)_{ac} G_\mu^b   \mathcal{E}^\mu_c
\end{align}
...
\begin{equation}
    \mathcal{D}_\mu \mathcal{D}_\nu G^{\nu\mu}_a = \frac{1}{2} [\mathcal{D}_\mu, \mathcal{D}_\nu] G^{\nu\mu}_a = 0 \, .
...
\begin{equation}
    \mathbb{G}_{\mu\nu} \propto [\mathcal{D}_\mu, \mathcal{D}_\nu] \, .
\end{equation}

\subsubsection*{3. Evaluando la curvatura (El Tensor Cromodinámico)}
Procedemos a evaluar explícitamente el conmutador de las derivadas covariantes en la representación fundamental ($\mathcal{D}_\mu = \partial_\mu - i g_s \mathbb{G}_\mu$) actuando sobre un campo de prueba:
\begin{align}
    [\mathcal{D}_\mu, \mathcal{D}_\nu] &= (\partial_\mu - i g_s \mathbb{G}_\mu)(\partial_\nu - i g_s \mathbb{G}_\nu) - (\partial_\nu - i g_s \mathbb{G}_\nu)(\partial_\mu - i g_s \mathbb{G}_\mu) \nonumber \\
    &= -i g_s (\partial_\mu \mathbb{G}_\nu - \partial_\nu \mathbb{G}_\mu) - g_s^2 [\mathbb{G}_\mu, \mathbb{G}_\nu] \, .
\end{align}
...
\begin{frame}[fragile,allowframebreaks]
El principio gauge local nos dictamina que para mantener la invarianza de la teoría, las derivadas parciales ordinarias deben ser promovidas a derivadas covariantes. En la Electrodinámica Cuántica (simetría abeliana $U(1)$), el tensor de campo electromagnético se construye a partir de derivadas normales:
\begin{equation}
    F_{\mu\nu} = \partial_\mu A_\nu - \partial_\nu A_\mu \, .
\end{equation}

Para generalizar este concepto a la Cromodinámica Cuántica (simetría no abeliana $SU(3)_c$), resulta natural agrupar los 8 campos de gluones $G^a_\mu$ en una única matriz $3 \times 3$ utilizando los generadores del grupo, $\Lambda_a$:
\begin{equation}
    {G}_\mu \equiv \Lambda_a G^a_\mu \, .
\end{equation}
...
\begin{align}
  \mathcal{D}_\mu G_\nu  = (\mathbf{1}\partial_\mu - i g_s {G}_\mu) {G}_\nu\,.
\end{align}

Inspirados por la forma del tensor electromagnético, postulamos que la matrix de tensores cromodinámicos, ${G}_{\mu\nu}$,  debe construirse reemplazando las derivadas normales por la \textbf{derivada covariante} anterior. Aplicando esta sustitución a la definición geométrica del tensor, tenemos:
\begin{align}
    {G}_{\mu\nu} &\equiv \mathcal{D}_\mu {G}_\nu - \mathcal{D}_\nu {G}_\mu \nonumber \\
    &= (\partial_\mu - i g_s {G}_\mu) {G}_\nu - (\partial_\nu - i g_s {G}_\nu) {G}_\mu \nonumber \\
    &= \partial_\mu {G}_\nu - \partial_\nu {G}_\mu - i g_s ({G}_\mu {G}_\nu - {G}_\nu {G}_\mu) \nonumber \\
    &= \partial_\mu {G}_\nu - \partial_\nu {G}_\mu - i g_s [{G}_\mu, {G}_\nu] \, .
    \label{eq:tensor_matricial_qcd}
\end{align}

Para extraer las componentes de color individuales $G^a_{\mu\nu}$, expandimos la Ecuación \eqref{eq:tensor_matricial_qcd} utilizando el álgebra de los generadores $[\Lambda_b, \Lambda_c] = i f_{abc} \Lambda_a$:
\begin{align}
    \Lambda_a G^a_{\mu\nu} &= \Lambda_a (\partial_\mu G^a_\nu - \partial_\nu G^a_\mu) - i g_s \left[ \Lambda_b G^b_\mu, \Lambda_c G^c_\nu \right] \nonumber \\
    &= \Lambda_a (\partial_\mu G^a_\nu - \partial_\nu G^a_\mu) - i g_s (i f_{abc} \Lambda_a) G^b_\mu G^c_\nu \, .
\end{align}
Factorizando $\Lambda_a$, obtenemos el tensor cromodinámico para cada componente de color:
\begin{equation}
    G^a_{\mu\nu} = \partial_\mu G^a_\nu - \partial_\nu G^a_\mu + g_s f^{abc} G^b_\mu G^c_\nu \, .
\end{equation}
Si el grupo fuera abeliano, las constantes de estructura $f^{abc}$ serían cero y recuperaríamos la forma exacta de $F_{\mu\nu}$. 
\end{frame}
...
\begin{align}
  {G}_{\mu\nu}\equiv & \mathcal{D}_{\mu} {G}_{\nu}-\mathcal{D}_{\nu} {G}_{\mu} \nonumber\\
=&\left( \mathbf{1}\partial_{\mu}-i g_s {G}_{\mu} \right){G}_{\nu}-
\left( \mathbf{1}\partial_{\nu}-i g_s {G}_{\nu} \right){G}_{\mu}\nonumber\\
=& \partial_{\mu}{G}_{\nu}-\partial_{\nu}{G}_{\mu}
-ig_s \left(  {G}_{\mu}{G}_{\nu}-{G}_{\nu}{G}_{\mu}  \right) \nonumber\\
=& \partial_{\mu}{G}_{\nu}-\partial_{\nu}{G}_{\mu}
-ig_s \left[  {G}_{\mu},{G}_{\nu}  \right] .
\end{align}
\end{frame}
