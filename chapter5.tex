...
\begin{frame}[fragile,allowframebreaks]
\subsection{Demostración compacta de la covarianza de $\mathbb{G}_{\mu\nu}$}

Existe una vía notablemente más simple y geométrica para demostrar que $\mathbb{G}'_{\mu\nu} = U \mathbb{G}_{\mu\nu} U^{-1}$, sin necesidad de expandir las derivadas explícitas del campo $\mathbb{G}_\mu$. Este método se basa en las propiedades de transformación del operador derivada covariante $\mathcal{D}_\mu$.

Por definición, el operador $\mathcal{D}_\mu$ garantiza que la derivada de un campo fundamental $\Psi$ transforme de la misma manera que el propio campo ($\Psi \to U\Psi$):
\begin{equation}
    \mathcal{D}'_\mu (U\Psi) = U (\mathcal{D}_\mu \Psi) \, .
\end{equation}
Multiplicando por la derecha por $U^{-1}$, deducimos que el operador en sí mismo transforma mediante una relación de similaridad pura:
\begin{equation}
    \mathcal{D}'_\mu = U \mathcal{D}_\mu U^{-1} \, .
\end{equation}

Recordando que el tensor de intensidad de campo se define geométricamente como el conmutador de las derivadas covariantes:
\begin{equation}
    \mathbb{G}_{\mu\nu} = \frac{i}{g_s} [\mathcal{D}_\mu, \mathcal{D}_\nu] \, ,
\end{equation}
evaluamos directamente la transformación del tensor sustituyendo $\mathcal{D}'_\mu$:
\begin{align}
    \mathbb{G}'_{\mu\nu} &= \frac{i}{g_s} \Big[ U \mathcal{D}_\mu U^{-1} , \, U \mathcal{D}_\nu U^{-1} \Big] \nonumber \\
    &= \frac{i}{g_s} \Big( U \mathcal{D}_\mu U^{-1} U \mathcal{D}_\nu U^{-1} - U \mathcal{D}_\nu U^{-1} U \mathcal{D}_\mu U^{-1} \Big) \, .
\end{align}
Dado que el producto $U^{-1}U$ en el interior de cada término es igual a la identidad ($\mathbf{1}$), la expresión se simplifica inmediatamente a:
\begin{align}
    \mathbb{G}'_{\mu\nu} &= \frac{i}{g_s} \Big( U \mathcal{D}_\mu \mathcal{D}_\nu U^{-1} - U \mathcal{D}_\nu \mathcal{D}_\mu U^{-1} \Big) \nonumber \\
    &= U \left( \frac{i}{g_s} [\mathcal{D}_\mu, \mathcal{D}_\nu] \right) U^{-1} \, .
\end{align}

Sustituyendo nuevamente la definición del tensor, concluimos la demostración de forma exacta:
\begin{equation}
    \mathbb{G}'_{\mu\nu} = U \mathbb{G}_{\mu\nu} U^{-1} \, .
\end{equation}
\textit{Nota:} La elegancia de esta demostración radica en que el comportamiento inhomogéneo del campo de gluones (los términos proporcionales a $\partial_\mu U$) ya está completamente absorbido y regularizado dentro de las propiedades del operador $\mathcal{D}_\mu$.
\end{frame}
...
\subsection{Calculemos ahora la transformación guage de $G_{\mu\nu}$}
\begin{align}
\label{eq:tdcg}
