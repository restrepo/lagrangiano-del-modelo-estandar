...
Aprovechando la arbitrariedad del término constante, podemos escribir el potencial en la forma más condensada
\begin{align}
  V(\phi)&=-\tfrac{1}{2}\lambda v^2\phi^2+\tfrac{1}{4}\lambda\phi^4+\tfrac{1}{4}\lambda v^4-\tfrac{1}{4}\lambda v^4\nonumber\\
  &=\tfrac{1}{4}\lambda \left(\phi^4-2 \phi^{2} v^2 +v^{4} \right)+\text{constant}\nonumber\\
    &=\tfrac{1}{4}\lambda \left(\phi^2-v^{2}\right)^{2}+\text{constant}\,.
\end{align}

Podemos escribir el potencial en ec.~\eqref{eq:eliminar} en términos del  nuevo campo como
\begin{align}
\label{eq:higgspot}
...
\label{eq:87qft}
   ...
   A_{\mu}\to A_{\mu}'=&A_{\mu}+\frac{1}{g}\partial_{\mu}\theta(x)\,.
\end{align}
\begin{align}
...
La masa del fotón dentro de un superconductor es $10^{-11}\,$GeV
(o 1/1000 de la masa del electrón de acuerdo a~\cite{massivephoton}).
También, del artículo en Beamline $\lambda\sim 10\ \mu\text{m}$ y $M_\gamma=\hbar/\lambda c$ %check numbers


Una revisión del uso de la ecuaciones de Proca para un fotón masivo en superconductividad se puede encontrar en~\cite{massivephoton}.
Un revisión divulgativa similar a la presentada aquí se puede encontrar en el libro de f Frank Wilczek ``The Lightness of Being''.
Ver:
